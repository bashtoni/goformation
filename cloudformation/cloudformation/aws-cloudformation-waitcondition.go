...
func (r *WaitCondition) UnmarshalJSON(b []byte) error {
	type P WaitCondition
	props := &WaitCondition{}
	newProps := &struct {
		*P
		Handle  types.StringIsh `json:"Handle,omitempty"`
		Timeout types.StringIsh `json:"Timeout,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.Handle = string(newProps.Handle)
		props.Timeout = string(newProps.Timeout)
