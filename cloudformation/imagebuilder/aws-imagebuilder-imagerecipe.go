...
func (r *ImageRecipe) UnmarshalJSON(b []byte) error {
	...
	newProps := &struct {
		*P
		...
		ParentImage      types.StringIsh `json:"ParentImage,omitempty"`
		Version          types.StringIsh `json:"Version,omitempty"`
		WorkingDirectory types.StringIsh `json:"WorkingDirectory,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.ParentImage = string(newProps.ParentImage)
		props.Version = string(newProps.Version)
		props.WorkingDirectory = string(newProps.WorkingDirectory)
