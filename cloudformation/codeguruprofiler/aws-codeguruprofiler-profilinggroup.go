...
func (r *ProfilingGroup) UnmarshalJSON(b []byte) error {
	type P ProfilingGroup
	props := &ProfilingGroup{}
	newProps := &struct {
		*P
		ComputePlatform    types.StringIsh `json:"ComputePlatform,omitempty"`
		ProfilingGroupName types.StringIsh `json:"ProfilingGroupName,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.ComputePlatform = string(newProps.ComputePlatform)
		props.ProfilingGroupName = string(newProps.ProfilingGroupName)
