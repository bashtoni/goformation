...
func (r *Stack) UnmarshalJSON(b []byte) error {
	...
	newProps := &struct {
		*P
		...
		FeedbackURL types.StringIsh `json:"FeedbackURL,omitempty"`
		Name        types.StringIsh `json:"Name,omitempty"`
		RedirectURL types.StringIsh `json:"RedirectURL,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.FeedbackURL = string(newProps.FeedbackURL)
		props.Name = string(newProps.Name)
		props.RedirectURL = string(newProps.RedirectURL)
