...
func (r *MaintenanceWindowTask) UnmarshalJSON(b []byte) error {
	type P MaintenanceWindowTask
	props := &MaintenanceWindowTask{}
	newProps := &struct {
		*P
		...
		TaskArn        types.StringIsh `json:"TaskArn,omitempty"`
		TaskType       types.StringIsh `json:"TaskType,omitempty"`
		WindowId       types.StringIsh `json:"WindowId,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.TaskArn = string(newProps.TaskArn)
		props.TaskType = string(newProps.TaskType)
		props.WindowId = string(newProps.WindowId)
