...
func (r *Listener) UnmarshalJSON(b []byte) error {
	...
	newProps := &struct {
		*P
		LoadBalancerArn types.StringIsh `json:"LoadBalancerArn,omitempty"`
		Protocol        types.StringIsh `json:"Protocol,omitempty"`
		SslPolicy       types.StringIsh `json:"SslPolicy,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.LoadBalancerArn = string(newProps.LoadBalancerArn)
		props.Protocol = string(newProps.Protocol)
		props.SslPolicy = string(newProps.SslPolicy)
