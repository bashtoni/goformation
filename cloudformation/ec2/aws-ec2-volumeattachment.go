...
func (r *VolumeAttachment) UnmarshalJSON(b []byte) error {
	type P VolumeAttachment
	props := &VolumeAttachment{}
	newProps := &struct {
		*P
		Device     types.StringIsh `json:"Device,omitempty"`
		InstanceId types.StringIsh `json:"InstanceId,omitempty"`
		VolumeId   types.StringIsh `json:"VolumeId,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.Device = string(newProps.Device)
		props.InstanceId = string(newProps.InstanceId)
		props.VolumeId = string(newProps.VolumeId)
