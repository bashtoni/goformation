...
func (r *Listener) UnmarshalJSON(b []byte) error {
	...
	newProps := &struct {
		*P
		AcceleratorArn types.StringIsh `json:"AcceleratorArn,omitempty"`
		ClientAffinity types.StringIsh `json:"ClientAffinity,omitempty"`
		Protocol       types.StringIsh `json:"Protocol,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.AcceleratorArn = string(newProps.AcceleratorArn)
		props.ClientAffinity = string(newProps.ClientAffinity)
		props.Protocol = string(newProps.Protocol)
