...
func (r *AggregationAuthorization) UnmarshalJSON(b []byte) error {
	type P AggregationAuthorization
	props := &AggregationAuthorization{}
	newProps := &struct {
		*P
		AuthorizedAccountId types.StringIsh `json:"AuthorizedAccountId,omitempty"`
		AuthorizedAwsRegion types.StringIsh `json:"AuthorizedAwsRegion,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.AuthorizedAccountId = string(newProps.AuthorizedAccountId)
		props.AuthorizedAwsRegion = string(newProps.AuthorizedAwsRegion)
