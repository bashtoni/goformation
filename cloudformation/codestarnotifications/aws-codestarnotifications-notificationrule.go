...
func (r *NotificationRule) UnmarshalJSON(b []byte) error {
	type P NotificationRule
	props := &NotificationRule{}
	newProps := &struct {
		*P
		DetailType types.StringIsh `json:"DetailType,omitempty"`
		Name       types.StringIsh `json:"Name,omitempty"`
		Resource   types.StringIsh `json:"Resource,omitempty"`
	...
	if res.Properties != nil {
		...
		props.DetailType = string(newProps.DetailType)
		props.Name = string(newProps.Name)
		props.Resource = string(newProps.Resource)
