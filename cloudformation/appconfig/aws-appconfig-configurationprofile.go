...
func (r *ConfigurationProfile) UnmarshalJSON(b []byte) error {
	...
	newProps := &struct {
		*P
		...
		LocationUri      types.StringIsh `json:"LocationUri,omitempty"`
		Name             types.StringIsh `json:"Name,omitempty"`
		RetrievalRoleArn types.StringIsh `json:"RetrievalRoleArn,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.LocationUri = string(newProps.LocationUri)
		props.Name = string(newProps.Name)
		props.RetrievalRoleArn = string(newProps.RetrievalRoleArn)
