...
func (r *ManagedPolicy) UnmarshalJSON(b []byte) error {
	type P ManagedPolicy
	props := &ManagedPolicy{}
	newProps := &struct {
		*P
		Description       types.StringIsh `json:"Description,omitempty"`
		ManagedPolicyName types.StringIsh `json:"ManagedPolicyName,omitempty"`
		Path              types.StringIsh `json:"Path,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.ManagedPolicyName = string(newProps.ManagedPolicyName)
		props.Path = string(newProps.Path)
