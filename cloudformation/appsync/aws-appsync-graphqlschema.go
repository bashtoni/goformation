...
func (r *GraphQLSchema) UnmarshalJSON(b []byte) error {
	type P GraphQLSchema
	props := &GraphQLSchema{}
	newProps := &struct {
		*P
		ApiId                types.StringIsh `json:"ApiId,omitempty"`
		Definition           types.StringIsh `json:"Definition,omitempty"`
		DefinitionS3Location types.StringIsh `json:"DefinitionS3Location,omitempty"`
	}{P: (*P)(props)}
	res := &struct {
		Type                string
	...
	if res.Properties != nil {
		...
		props.Definition = string(newProps.Definition)
		props.DefinitionS3Location = string(newProps.DefinitionS3Location)
